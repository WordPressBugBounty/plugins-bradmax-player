	...
	class Bradmax_Player_Plugin {
		...
		static public function bradmax_player_enqueue_scripts() {
			if (!is_admin()) {
				...
				wp_register_style('bradmax-player', $plugin_url . self::DEFAULT_CSS_STYLES_FILE_PATH, array(), self::PLUGIN_VERSION);
		...
		static private function upload_customized_player_js($file_field) {
			global $wp_filesystem;
			check_admin_referer( 'bradmax_player_file' );
			WP_Filesystem();
			if(empty($_FILES[$file_field])) {
				return array('error' => 'File cannot be uploaded into server (no expected field).');
			}
			$file_rec = wp_handle_upload($_FILES[$file_field],
				array(
					'test_form' => false,
					'test_type' => false,
					'test_size' => true,
			...
			if(isset($file_rec['error'])) {
				return $file_rec;
			}

			// Check file extension.
			if(!isset($file_rec['file']) || !preg_match('/\.js$/i', sanitize_file_name($file_rec['file']))) {
				$wp_filesystem->delete($file_rec['file']);
			...
			$content = $wp_filesystem->get_contents($file_rec['file']);
			$wp_filesystem->delete($file_rec['file']);
			...
			if(!$wp_filesystem->put_contents(self::get_customized_player_file_path(), $content)) {
		...
		static private function get_customized_player_info() {
			...
			// Read player file.
			$content = $wp_filesystem->get_contents(self::get_customized_player_file_path());
		...
		static public function options_page() {
			...
			if(isset($_FILES['bradmax_player_file'])) {
				...
				$upload_info = self::upload_customized_player_js('bradmax_player_file');
			}

			if(isset($_REQUEST['remove_player'])) {
				check_admin_referer( 'remove_player' );
				// Remove customized player.
				wp_delete_file(self::get_customized_player_file_path());
			}
...
"					<div id=\"$player_wrapper_id\" style=\"$player_wrapper_style_str\" $player_wrapper_class_str></div>\n";
		...
		static private function bradmax_build_time_markers_and_chapters_config($params) {
			...
			if(!empty($params['chapters'])) {
				$segments = array();
				$parts = preg_split('/[\n\r;]+/', wp_strip_all_tags($params['chapters']));
			...
			if(!empty($params['time_markers'])) {
				$markers = array();
				$parts = preg_split('/[\n\r;]+/', wp_strip_all_tags($params['time_markers']));
				foreach($parts as $line) {
					$m = null;
					if(!preg_match('/(([0-9]+:)?([0-9]+:)?[0-9]+) *-? *(.*)/', $line, $m)) {
						continue;
		...
		static public function bradmax_video_embed_handler($atts) {
			...
			$player_config_str = wp_json_encode($player_config);
